...
exports.getScript = (req, res, next) => 
{
  ...
  else 
  {
    ...
    //Get the newsfeed
    Script.find()
     // .where("experiment_group").equals(scriptFilter) //no content/actor filtering 
      ...
      .exec(function (err, script_feed) 
      {
        ...
        while(script_feed.length || user_posts.length) 
        {
          ...
          if(typeof script_feed[0] === 'undefined') 
          {
              ...
              if(!user_posts[0].hidden) //Filter out hidden posts
              {
            ...
            if(!(typeof user_posts[0] === 'undefined') && (script_feed[0].time < user_posts[0].relativeTime))
            {
              console.log("Push user_posts");  //Filter out hidden posts
              if (!user_posts[0].hidden)
              {
                finalfeed.push(user_posts[0])
              }
              user_posts.splice(0,1);      
            ...
            if(feedIndex!=-1)
            {
                    ...
                    else
                    {
                      ...
                      if(commentIndex!=-1)
                      {
                        ...
                        { 
                          ...
                          script_feed[0].comments[commentIndex].flagged++;
              ...
              { 
                ...
                if(user.group=='consensus_lc')
                  script_feed[0].dislikes_lc++;
                if(user.group=='consensus_hc')
                  script_feed[0].dislikes_hc++;
                console.log("Post %o has been DISLIKED", script_feed[0].id);
              }

              if (!user.feedAction[feedIndex].disliked)
              { 
                script_feed[0].dislike = false;
                if(user.group=='consensus_lc')
                  script_feed[0].dislikes_lc;
                if(user.group=='consensus_hc')
                  script_feed[0].dislikes_hc;
                console.log("Post %o has been UNDISLIKED", script_feed[0].id);
              }
...
/******************************************************************************************************************
 *                        [Testing Purposes]   GET list of Script posts for Feed  
******************************************************************************************************************/
...
exports.postUpdateFeedAction = (req, res, next) => 
{
  User.findById(req.user.id, (err, user) => 
  {
    ...
    else if(req.body.commentID)
    {
      ...
      if(req.body.like)
      {
        console.log("Comment ID: ", commentIndex); // User likes an actor's comment in an actor's post
    ...
    }//end of actions on comments
    
    else //not a comment - its a post action
    { 
...
}; //end of Post update feed action

/******************************************************************************************************************* 
 *       	      	POST /pro_feed/ | Update user's profile feed posts Actions. getUserPostByID
 *****************************************************************************************************************/
...
};//end of post update pro feed action

/*****************************************************************************************************************
 *                            userPost_feed/ | Update user's POST feed Actions.
 *****************************************************************************************************************/
exports.postUpdateUserPostFeedAction = (req, res, next) => 
{
  User.findById(req.user.id, (err, user) => 
  {
    ...
    else if(req.body.new_comment) //create a new Comment
    {
        var cat = new Object();
        cat.new_comment = true;
        user.numReplies = user.numReplies + 1;
    ...
    else if(req.body.commentID) //Are we doing anything with a comment?
    {
      ...
      {
        console.log("%^%^%^%^%^%User Post comments LIKE wasSAA: ", user.posts[feedIndex].comments[commentIndex].liked);
    ...
    }//end of all comment stuff. 

    else // Need to deal with post actions (Like/Hide).
    {
      //we found the right post, and feedIndex is @ the right position.
      console.log("##### FOUND post "+req.body.postID+" at index "+ feedIndex);
      ...
      {     
        console.log("!!!!!!User Post Hide was: ", user.posts[feedIndex].hidden);
        user.posts[feedIndex].hidden = user.posts[feedIndex].hidden ? false : true;
        //user.feedAction[feedIndex].hidden = user.posts[feedIndex].hidden;
        console.log("!!!!!!User Post Hide is now: ", user.posts[feedIndex].hidden);
        console.log("@@@Pushing the hiding stuff into the log");
        
	var log = {};
        log.time = Date.now();
        log.postID = req.body.postID;
        //log.hide_issue = req.body.hide_issue;
        //console.log("hide_issue is:",req.body.hide_issue);
        user.hidePostLog.push(log);
        }        
      else
      {
        console.log("Got a POST that did not fit anything. Possible Error.")

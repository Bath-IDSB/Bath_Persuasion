...
/*************************************************************************************************************
*                                        Get all actors data from DB (testing)
**************************************************************************************************************/
    ...
    if (!(user.active))
    {
	   ...
	   Actor.findOne({ username: req.params.userId}, (err, act) => 
     {
        ...
        if (act == null) 
        {
            ...
            var myerr = new Error('Record not found!'); return next(myerr);
        }
        ...
        if (user.blocked.includes(req.params.userId)) //check if the actor is blocked
        {
          isBlocked = true;
        }
        else
        ...
        .exec(function (err, script_feed) 
          {
           ...
           {
                var feedIndex = _.findIndex(user.feedAction, function(o) 
                    { 
              ...
              if(feedIndex!=-1)
              {
                //console.log("WE HAVE AN ACTION!!!!!");
                  ...
                  if (!user.feedAction[feedIndex].liked)
                  { 
                    script_feed[0].like = false;
                    script_feed[0].likes;
                    console.log("Post %o has been UNLIKED", script_feed[0].id);
                  }
...
/***************************************************************************************************************
 *                                   Update Feed/ User's actor-based actions (Block,Unblock,Report & Message).
 ***************************************************************************************************************/
exports.postBlockOrReport = (req, res, next) => 
{
  User.findById(req.user.id, (err, user) => 
  {
    ...
    else if (req.body.messaged)
    {
      console.log("@@@Private messaging a user now")
      user.messaged.push(req.body.messaged);
      var log = {};
      log.time = Date.now();
      log.action = "message";
      log.actorName = req.body.messaged;
      log.pvt_message = req.body.pvt_message;
      user.blockAndReportLog.push(log);
    }

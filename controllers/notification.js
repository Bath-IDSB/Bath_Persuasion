...
/*******************************************************************************************************************
 *                                GET / | List of Script posts for Notification Feed
********************************************************************************************************************/
...
/*User.findById(req.user.id).exec(function (err, user) 
  ...
  .exec(function (err, user) 
  {
    ...
    if (user.script_type == "study3_n80")
    {
        scriptFilter = "study3_n80";
        profileFilter = "study_n80_p60";
    }
    user.logPage(Date.now(), "Notifications"); //Log our visit to Notifications
    user.lastNotifyVisit = Date.now();

    console.log("Past user_posts now"); //Also get all reply posts as well
    if (user.posts.length == 0)
    {
        console.log("No User Posts yet. Sending to -1 to PUG"); //peace out - send empty page,  
        res.render('notification', { notification_feed: -1 }); //or deal with replys or something IDK
    }
    else
          ...
          for (var i = 0, len = notification_feed.length; i < len; i++) 
          {
            ...
            if (notification_feed[i].userPost >= 0)
            {
              ...
              { 
                ...
                else if (notification_feed[i].notificationType == "reply") // If notification REPLY
                {

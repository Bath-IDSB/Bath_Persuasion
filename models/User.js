...
const userSchema = new mongoose.Schema({
  ...
  email: { type: String},
  prolificID: { type: String, unique: true},
  password: String,
  ...
  study_days: {type: [Number],default: [0, 0, 0]},
...
/*userSchema.pre('save', function save(next) 
{
  ...
  bcrypt.genSalt(10, (err, salt) => 
  {
    ...
    bcrypt.hash(user.prolificID, salt, (err, hash) => 
    {
      if (err) 
        { 
          return next(err); 
        }
      user.prolificID = user.prolificID 
      next();
    });
  });
});*/
...
{
  bcrypt.compare(candidatePassword, this.prolificID, (err, isMatch) => 
  {
    cb(err, isMatch);
  });
};*/
...
userSchema.methods.getPosts = function getPosts() 
{
  ...
  for (var i = 0, len = this.posts.length; i < len; i++) 
  {
    ...
    {
      //this.posts[i].comments.splice(0,); //if you want to display post without comments
...
userSchema.methods.getPostsAndReplies = function getPostsAndReplies() 
{
  ...
  for (var i = 0, len = this.posts.length; i < len; i++) 
  {
    //if(!(this.posts[i].hidden)) //if we want to exculude hidden posts as well
    //{
...
//Return the actor reply from its ID
